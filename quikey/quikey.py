...
@click.option(
    ...
    "-t",
    multiple=True,
    help="Optional tags for the phrase. You can specify this option multiple times.",
)
@click.option(
    "--phrase",
    "-p",
    help="The full phrase to add. If this option is not specified then your default editor ($EDITOR) will be used.",
)
@click.pass_context
...
def edit(ctx, name):
    ...
    if name is None:
        ...
        if name is None:
            click.echo(
                'No phrases available to edit. Use "qk add" to create a new one.'
            )
            return
...
@click.option(
    "--show-all",
    is_flag=True,
    help="Show the entire quikey phrase instead of a shortened version "
...
def ls(ctx, show_all):
    ...
    for phrase in phrases:
        ...
        table.append(
            [phrase.get("key"), tags, humanize.naturalday(phrase.get("updated")), value]
        )
...
@click.option(
    "--location",
    "-l",
    default=BaseDirectory.xdg_config_home + "/autokey/data/",
    show_default=True,
    help="Location of top level directory to import from autokey",
)
@click.pass_context
